...
void init(OptionsMap& o) {
    ...
    o["ManualWeights"] << Option(false, [](const Option& opt) {
    // Check if Manual Weights option is enabled or disabled.
     if (opt) {
        sync_cout << "info string ManualWeights enabled" << sync_endl;
    } else {
        sync_cout << "info string ManualWeights disabled" << sync_endl;
    }
});

    o["StrategyMaterialWeight"] 
        << Option(0, -12, 12, on_strategy_material_weight); // Material weight adjustment.
    o["StrategyPositionalWeight"] 
    ...
    o["NNUE Dynamic Weights"] << Option(true, [](const Option& opt) {
